...
<!-- generated by tables.py: classifier -->
...
| classifier + veto at 0.816 (tuned) | 0.988     | 0.960  | 0.974     |
<!-- end -->
...
test fold than a naive 0.5: it buys 0.011 precision for 0.034 recall, and F1
falls from 0.986 to 0.974. Tuning on one fold does not automatically transfer,
...
features, one split). The reference row is the model *without* the volume veto,
which is why it reads 0.977 rather than the 0.986 bolded above: the veto is a
constraint applied after scoring, not a feature, so it is held out of a table
about features. That reference is bit-identical to the second row of the
classifier comparison, and a test asserts it rather than trusting the coincidence:
...
20260845, flat threshold 0.5, veto applied, candidate table built once), every
figure below is a median with full range across splits. These three tables are
generated by `tables.py` from `results/sweep.json`, and `test_tables.py` fails
when the committed README disagrees with what the generator renders, so they
cannot go stale by hand.
...
counted over the whole candidate table rather than the fold: a one-to-one policy
has to see the pairs competing for a record before it can reject any of them, so
`sweep.py` applies each policy to all 3750 pairs and scores precision and recall
on the held-out slice of that decision.
...
Ten tables are generated from a recorded run and a test fails when they drift:
the three sweep tables, the feature ablation, blocking recall, the volume
comparison, the classifier comparison, Abt-Buy blocking, the Abt-Buy classifier
and the Abt-Buy feature ablation. That is every table in this file. Prose figures
are still typed by hand - most of the ones carrying an argument are pinned by a
test against the same recording, but not all of them are, so the failure mode is
narrowed rather than closed. It has already cost this README twice - four numbers
went stale, and three findings that were really one split got published as
results.
