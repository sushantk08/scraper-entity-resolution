...
Seven tables are generated from a recorded run and a test fails when they drift:
the three sweep tables, the feature ablation, blocking recall, the volume
comparison and Abt-Buy blocking. Three are not - the classifier comparison and
two more Abt-Buy tables are still typed by hand from console output, as are prose
