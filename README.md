...
Eight tables are generated from a recorded run and a test fails when they drift:
the three sweep tables, the feature ablation, blocking recall, the volume
comparison, Abt-Buy blocking and the Abt-Buy classifier. Two are not - the books
classifier comparison and the Abt-Buy feature ablation are still hand-typed from
console output, as are prose
figures like the fold sizes. That is the same failure mode, still open. It has
already cost this README twice - four numbers went stale, and three findings that
were really one split got published as results.
